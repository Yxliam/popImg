...
;(function($, window, undefined) {
  ...
  PopImg.prototype = {
    init:function(){
      ...
      this.zoomImgBox   = 'mobile-gallery-img';
      this.zoomPop      = 'yxl-zoom';
      this.popElement   = 'mobile-gallery';
      this.scriWidth    = $(window).width();
      this.scriHeight   = $(window).height(); 
      this.zoomNum      = 'mobile-gallery-num';
      try {
          this.imgAddStyle();
          this.bindClick();
      } catch(e) {
    ...
    bindClick:function(){
      var _this = this;
      $(this.body).on('click','.yxl-zoom',function(e){
         e.preventDefault();
         var target   = e.target;
      ...
      $(this.body).on('click','.mobile-gallery',function(e){
          e.preventDefault();
      })
    },
    zoom:function(){
        var _imgBox = '',
            _imgWrapWidth = this.imgLen * this.scriWidth;
        for(var i = 0;i < this.imgLen; i++){
        //    var _top = ((this.scriHeight)/2) - ($(this.imgArr[i]).height()/2);
        //    if( _top < 0)  _top = 0;
           _imgBox +=' <div class="mobile-gallery-img" style="width:'+this.scriWidth+'px">\
                            <img class="'+this.zoomImg+'" src="'+$(this.imgArr[i]).attr("src")+'" alt="">\
                        </div>';
        }
         this.pop ='<div class="mobile-gallery">\
                        <div class="mobile-gallery-counter"><span class="mobile-gallery-num">'+(parseInt(this.index)+1)+'</span>/'+this.imgLen+'</div>\
                        <div class="mobile-gallery-group" style="width:'+_imgWrapWidth+'px;transform: translateX(-'+(this.scriWidth*this.index)+'px);">\
                            '+_imgBox+'\
                        </div>\
                    </div>';
          $(this.pop).appendTo($(this.body));  
          this.slideHandle();
          $(this.body).css({height:'100%','overflow':'hidden','position':'fixed'})
          this.scale('.'+this.zoomImgBox);
    },
    slideHandle:function(e){
        var self = this;
        touch.on('.'+self.zoomImgBox,'swipeleft',function(e){
            self.scaleNoneHandle();
            self.slideLeft();
        })
        touch.on('.'+self.zoomImgBox,'swiperight',function(e){
            self.scaleNoneHandle();
            self.slideRight();
        })
        touch.on('.'+self.zoomImgBox,'tap',function(e){
            self.closePop();
        })  
        touch.on('.'+self.zoomImgBox, 'swipedown', function (ev) {
            self.closePop();
        });
    },
    scaleNoneHandle:function(){
        $('.'+this.zoomImg).css({transform:'scale(1)','-webkit-transform':'scale(1)'});
    },
    closePop:function(){
        $('.'+this.popElement).remove();
        $(this.body).css({'overflow':'auto','position':'static'})
    },
    slideLeft:function(){
        if(parseInt(this.index)+1 >= (this.imgLen)) return
        this.index = parseInt(this.index) + 1
        $('.mobile-gallery-group').css({transform: "translateX(-"+(this.scriWidth*this.index)+"px)",transition: "all 0.3s"})
        $('.'+this.zoomNum).text((this.index+1));
    },
    slideRight:function(){
        if(parseInt(this.index) === 0) return
        $('.mobile-gallery-group').css({transform: "translateX(-"+(this.scriWidth*(this.index-1))+"px)",transition: "all 0.3s"})
        $('.'+this.zoomNum).text((this.index));
        this.index = parseInt(this.index-1)
    },
    scale: function ($targetObj) {
        ...
        touch.on($targetObj, 'pinch', function (ev) {
            ...
            if(this.scaleVal < 0.8 ){
                this.scaleVal = 0.8
            }
            var transformStyle = 'scale(' + this.scaleVal + ')';
            $('.'+self.zoomImg).css("transform", transformStyle).css("-webkit-transform", transformStyle);
        });
